...
(function($){
  ...
  BioCircos.prototype.draw_genome = function(genome){
    ...
    var chord = d3.chord()
      .padAngle(self.chrPad)
      .sortSubgroups(d3.descending)(genome);
    ...
    if(self.zoom == true){
        var zoom = d3.zoom().scaleExtent([0.9, 10]).on("zoom", zoomed);
        ...
        function zoomed() {
            ...
            svg.attr("transform", d3.event.transform.translate(width/2,height/2));
        }
        ...
        var button = d3.select(self.target).select("svg")
            .append("g")
            .attr("transform", "translate(" + (width-1) + "," + (height - 31) + ")")
        ...
        for(var backgroundi=0; backgroundi<self.BACKGROUND.length; backgroundi++){
            ...
            svg.append("g").selectAll("path")
              ...
              .enter().append("path")
                ...
                .attr("d", d3.arc().innerRadius(self.BACKGROUNDsettings.BginnerRadius).outerRadius(self.BACKGROUNDsettings.BgouterRadius));
                ...
                for(i=1;i<=self.BACKGROUNDsettings.axisNum;i++){
                    svg.append("g").selectAll("path")
                      ...
                      .enter().append("path")
                        ...
                        .attr("d", d3.arc().innerRadius(self.BACKGROUNDsettings.BginnerRadius+(self.BACKGROUNDsettings.BgouterRadius-self.BACKGROUNDsettings.BginnerRadius)/(self.BACKGROUNDsettings.axisNum+1)*i).outerRadius(self.BACKGROUNDsettings.BginnerRadius+(self.BACKGROUNDsettings.BgouterRadius-self.BACKGROUNDsettings.BginnerRadius)/(self.BACKGROUNDsettings.axisNum+1)*i+self.BACKGROUNDsettings.axisWidth));
                }
            }
    ...
    if(self.CNV.length > 0){
        ...
        for(var cnvi=0; cnvi<self.CNV.length; cnvi++){
            ...
            svg.append("g")
                ...
                .attr("d", function(d,i) { var cnv = d3.arc().innerRadius(self.CNVsettings.minRadius+d.cnv_deviation).outerRadius(self.CNVsettings.minRadius+self.CNVsettings.CNVwidth+d.cnv_deviation); return cnv(d,i); });
    ...
    if(self.HEATMAP.length > 0){
        ...
        for(var heatmapi=0; heatmapi<self.HEATMAP.length; heatmapi++){
            ...
            var heatmap = d3.arc().innerRadius(innerRadius+self.HEATMAPsettings.innerRadius).outerRadius(outerRadius+self.HEATMAPsettings.outerRadius);
            svg.append("g")
    ...
    if(self.SCATTER.length > 0){
        ...
        for(var scatteri=0; scatteri<self.SCATTER.length; scatteri++){
            ...
            .ease(d3["ease" + self.SCATTERsettings.SCATTERAnimationType[0].toUpperCase() + self.SCATTERsettings.SCATTERAnimationType.slice(1)])
                    .attr("cx", function(d) { return d.x; })
                    .attr("cy", function(d) { return d.y; });
               }
    ...
    if(self.SNP.length > 0){
        ...
        for(var snpi=0; snpi<self.SNP.length; snpi++){
            ...
            .ease(d3["ease" + self.SNPsettings.SNPAnimationType[0].toUpperCase() + self.SNPsettings.SNPAnimationType.slice(1)])
                    .attr("cx", function(d) { return d.x; })
                    .attr("cy", function(d) { return d.y; });
               }
    ...
    if(self.LINK.length > 0){
        ...
        for(var linki=0; linki<self.LINK.length; linki++){
            ...
            if(self.LINKsettings.LinkAnimationDisplay==true){
                ...
                .ease(d3["ease" + self.LINKsettings.LinkAnimationType[0].toUpperCase() + self.LINKsettings.LinkAnimationType.slice(1)])
                        .attr("opacity",self.LINKsettings.LinkOpacity);
            }
